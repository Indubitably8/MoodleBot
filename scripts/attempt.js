...
async function main() {
  ...
  if (questionElements.length != 0) {
    for (let i = 0; i < questionElements.length; i++) {
      var filteredText = (questionIds[i] = Number(
        questionElements[i].parentNode.parentNode.parentNode.id.split("-")[2]
      ));
      questions.push(filterText(questionElements[i].textContent));
    }
  }
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    button.addEventListener("click", function onButtonClick() {
      questionElements[i].textContent =
        questionElements[i].textContent + " == " + FindAnswer(questions[i]);
      button.remove();
      questionElements[i].textContent = questionElements[i].textContent.replace(
        "Reveal Answer",
        ""
      );
    });
    ...
    for (let i = 0; i < inputs.length; i++) {
      if (inputs[i].value.includes("`")) {
        document
          .getElementById(
            "button" +
              String(Number(inputs[i].id.slice(9).replace("_answer", "")) - 1)
          )
          .click();
        inputs[i].value = "";
      }
  ...
  function auto(options = {}) {
      ...
      if (correct != correctGoal) {
        if (
          FindAnswer(
            questions[
              questionIds.indexOf(
                Number(inputs[i].id.slice(9).replace("_answer", ""))
              )
            ]
          ) != "IDFK"
        ) {
    ...
    setTimeout(
      () => document.getElementById("mod_quiz-next-nav").click(),
      (inputs.length * pauseInterval) / speed + 1000
    );
    ...
    var display =
      ...
      ";   " +
      time +
      " Seconds to Complete;   Unknown Answers: " +
      nullCount;
    return display;
  }
  ...
  function numberPassed() {
    ...
    for (let i = 0; i < text.length; i++) {
      if (Number(text[i].textContent) >= 8) {
        if (
          text[i].parentNode.textContent.includes(
            dates[dates.length - 2].textContent.substring(0, 30)
          )
        ) {
          result++;
        }
